...
async function createArticleHeadTagInstanceMiddleware(req, res, next) {
  ...
  ret_article_head_tag_instance = new ArticleHeadTag({
    ...
    noindex: req.body.noindex ? req.body.noindex : undefined,
    // [req.body.nofollow ? 'nofollow' : null]: req.body.nofollow, // not required and defaults to false
    nofollow: req.body.nofollow ? req.body.nofollow : undefined,
    article_id: res.locals.article._id // ATTACH TO ArticleHeadTag -> Article
  })


  res.locals.article.articleheadtag_id = ret_article_head_tag_instance._id // ATTACH TO Article -> ArticleHeadTag
...
async function createArticleBodyHeaderInstanceMiddleware(req, res, next) {
  ...
  ret_article_body_header_instance = new ArticleBodyHeader({
    ...
    article_id: res.locals.article._id // ATTACH TO ArticleBodyHeader -> Article
  })

  res.locals.article.articlebodyheader_id = ret_article_body_header_instance._id // ATTACH TO Article -> ArticleBodyHeader
...
async function createArticleAbstractMiddleware(req, res, next) {
  ...
  ret_article_abstract_instance = new ArticleAbstract({
    ...
    article_id: res.locals.article._id,  // ATTACH TO ArticleAbstract -> Article
  })


  res.locals.article.articleabstract_id = ret_article_abstract_instance._id // ATTACH TO Article -> ArticleAbstract
...
async function createArticleNestedDatatMiddleware2(req, res, next) {
  ...
  ret_article_nested_data_instance = new ArticleNestedData({
    ...
    article_id: res.locals.article._id
  });




  res.locals.article.articlenesteddata_id = ret_article_nested_data_instance._id;
